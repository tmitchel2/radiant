...
generates their distance fields (`MsdfGenerator`), but it doesn't touch the GPU:
`Renderer2D.DrawParagraph` draws a paragraph from an atlas of either (see
...
- **More ways to draw glyph runs:** Slug. `Renderer2D.DrawText` still draws from the baked MSDF
  atlases.
- **Line layout:**
