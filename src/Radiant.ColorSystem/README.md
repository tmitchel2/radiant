...
Radiant's colour system: the HCT colour space, tonal palettes, dynamic schemes and their colour
roles, contrast, blending, and extracting a source colour from an image.

It is a C# port of Google's
[material-color-utilities](https://github.com/material-foundation/material-color-utilities)
(Apache-2.0; see `LICENSE` here). The algorithms are upstream's. The names are Radiant's: nothing
public is named for the upstream design system.
...
- **Deprecated upstream APIs are not ported:** the legacy `Scheme` and `CorePalette`, and
  upstream's `MaterialDynamicColors` static fields.
...
| dynamiccolor | `DynamicColor`, `DynamicScheme`, `RadiantDynamicColors`, `ContrastCurve`, `ToneDeltaPair`, `Variant`, `SpecVersion`, `Platform`, and the internal color specs 2021/2025/2026 |
...
int any = scheme.GetArgb(RadiantDynamicColors.SurfaceContainerHigh()); // or any dynamic color
```

## Where the C# API differs

- **Renamed to Radiant:**
  - the project and namespace (upstream's package is `material-color-utilities`);
  - `MaterialDynamicColors` is `RadiantDynamicColors`;
  - the color specs are `ColorSpec2021`/`2025`/`2026` (upstream's `ColorSpecDelegateImpl*`).
- `Score.score` is `Score.Rank`: C# cannot name a method after its class.
- `RadiantDynamicColors` is a static class. Upstream's instance methods build a new
...
`tools/color-system-fixtures/generate.sh` checks out the pinned commit, builds its TypeScript (and Java, for
quantization), and records its outputs over a spread of inputs in
`src/Radiant.ColorSystem.Tests/Fixtures`. The tests require every colour to match exactly. Doubles
...
2. Re-translate the color specs with `tools/color-system-fixtures/transpile-color-specs.py`, then
