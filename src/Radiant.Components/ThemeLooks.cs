...
internal static class ThemeLooks
{
    ...
    /// <summary>The empty part of slider and progress tracks.</summary>
    public static Radiant.Graphics2D.Color Track(this ResolvedTheme theme)
    {
        var style = theme.Theme.Components.Selection;
        return theme.Get(style.Track, container: style.TrackContainer);
    }
    ...
    /// <summary>The raised pill of a chosen segment: the bright surface on a light theme, a lighter tint of the surface on a dark one.</summary>
    public static Radiant.Graphics2D.Color SegmentPill(this ResolvedTheme theme, SurfaceState surface) =>
        theme.Theme.Colors.IsDark ? theme.StateLayerColor(surface, 0.14f) : theme.Get(SurfaceName.SurfaceBright);
}
