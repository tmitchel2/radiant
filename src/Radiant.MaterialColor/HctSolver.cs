...
/// <summary>
/// Solves the inverse of the CAM16 color appearance model: the sRGB color with a given HCT hue,
/// chroma and tone, reducing chroma as little as possible when the requested one is out of gamut.
/// <para>
/// Tries the exact answer first (Newton's method on CAM16 lightness J). Failing that, it bisects the
/// sRGB gamut boundary at the requested tone: along the edges of the cube's cross-section, then
/// through the "critical planes" where an 8-bit channel value changes, for the in-gamut color
/// nearest the requested hue.
/// </para>
/// </summary>
public static class HctSolver
{
    private static readonly double[][] s_scaledDiscountFromLinrgb =
    [
    ...
    private static readonly double[][] s_linrgbFromScaledDiscount =
    [
    ...
    private static readonly double[] s_yFromLinrgb = [0.2126, 0.7152, 0.0722];

    // Linear-RGB values (0–100) midway between consecutive 8-bit sRGB values: where delinearized
    // output steps from one integer to the next.
    private static readonly double[] s_criticalPlanes =
    [
    ...
    /// <returns>
    /// A hexadecimal representing the sRGB color. The color has sufficiently close hue, chroma and
    /// L* to the desired values, if possible; otherwise, the hue and L* will be sufficiently close,
    /// and chroma will be maximized.
    /// </returns>
    ...
    /// <summary>As <see cref="SolveToInt"/>, returned as a <see cref="Cam16"/>.</summary>
    ...
    // Sanitizes a small enough angle in radians: 0 (inclusive) to 2π (exclusive).
    private static double SanitizeRadians(double angle) => (angle + Math.PI * 8) % (Math.PI * 2);

    // Delinearizes an RGB component, returning a floating-point number: 0.0–100.0 linear in,
    // 0.0–255.0 (not rounded) out.
    ...
    {
        var scaledDiscount = MathUtils.MatrixMultiply(linrgb, s_scaledDiscountFromLinrgb);
    ...
    // How far along the source→target segment mid lies: 0 at source, 1 at target.
    ...
    // The point on the source→target segment whose given axis equals coordinate.
    ...
    // The nth possible vertex of the polygonal intersection of the RGB cube and the plane of
    // constant Y, or [-1, -1, -1] when that vertex lies outside the cube.
    private static double[] NthVertex(double y, int n)
    {
        var kR = s_yFromLinrgb[0];
        var kG = s_yFromLinrgb[1];
        var kB = s_yFromLinrgb[2];
    ...
    // The segment of the constant-Y cross-section's boundary containing the target hue.
    ...
    // The in-gamut color at luminance y nearest the target hue, found by bisecting through the
    // critical planes of each axis in turn.
            ...
            if (left[axis] != right[axis])
            {
                ...
                for (var i = 0; i < 8; i++)
                {
                    ...
                    else
                    {
                        ...
                        var midPlaneCoordinate = s_criticalPlanes[mPlane];
    ...
    // The exact in-gamut color, by Newton's method on CAM16 lightness; 0 when there is none.
    private static int FindResultByJ(double hueRadians, double chroma, double y)
    {
        ...
        for (var iterationRound = 0; iterationRound < 5; iterationRound++)
        {
            ...
            var linrgb = MathUtils.MatrixMultiply([rCScaled, gCScaled, bCScaled], s_linrgbFromScaledDiscount);
            // ===========================================================
            // Operations inlined from Cam16 to avoid repeated calculation
            // ===========================================================
