#!/usr/bin/env python3
"""Translates upstream's color_spec_{2021,2025,2026}.ts into Radiant.MaterialColor's ColorSpec*.cs.

    python3 transpile-color-specs.py <upstream>/typescript/dynamiccolor src/Radiant.MaterialColor
    dotnet format whitespace src/Radiant.MaterialColor/Radiant.MaterialColor.csproj
...
{usings}namespace Radiant.MaterialColor;

/// <summary>The Material color roles as the {cls_name[-4:]} spec defines them. Upstream's <c>ColorSpecDelegateImpl{cls_name[-4:]}</c>.</summary>
